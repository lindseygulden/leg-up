            ...
            ts: dict(
                zip(
                    ...
                    [
                        ...
                        * (1 - 2 * self.df.loc[e, "capture_fraction"])
                        for e in self.energy_sources
                    ],
                )
            )
...
class NestedLogitIAM(IAM):
    ...
    def compute_new_shares(self):
        ...
        calc_df = pd.DataFrame(
            ...
            index=["subsector", "subsector_share", "price"],
        ).T
        calc_df.index.name = "energy_source"
        calc_df["share_times_price"] = calc_df["subsector_share"] * calc_df["price"]

        calc_subsec_totals_df = calc_df.groupby("subsector").sum().reset_index()

        calc_subsec_totals_df.columns = [
            "subsector",
            "subsector_total_share",
            "subsector_avg_price",
            "share_times_price",
        ]
        calc_subsec_totals_df["subsector_avg_price"] = (
            calc_subsec_totals_df["share_times_price"]
            / calc_subsec_totals_df["subsector_total_share"]
        )
        calc_df = calc_df.reset_index().merge(
            calc_subsec_totals_df[
                ["subsector", "subsector_total_share", "subsector_avg_price"]
            ],
            on="subsector",
        )
        calc_df["share_of_subsector_total"] = (
            calc_df["subsector_share"] / calc_df["subsector_total_share"]
        )

        # compute subsector shares
        share_of_subsector_dict = {}

        for subsector in self.logit_exponents["subsector"].keys():
            calc_subset_df = calc_df.loc[calc_df.subsector == subsector].set_index(
                "energy_source"
            )
            calc_subset_df["shares"] = self._modified_logit_shares(
                calc_subset_df["share_of_subsector_total"],
                calc_subset_df["price"],
                self.logit_exponents["subsector"][subsector],
            )
            share_of_subsector_dict = share_of_subsector_dict | (
                calc_subset_df["shares"].to_dict()
            )

        calc_grouped_subset_df = (
            calc_df.groupby("subsector")
            .first()[["subsector_avg_price", "subsector_total_share"]]
            .copy()
        )
        ...
        for sector in self.logit_exponents["sector"].keys():
            calc_grouped_subset_df["new_sector_share"] = self._modified_logit_shares(
                calc_grouped_subset_df["subsector_total_share"],
        ...
        calc_df.set_index("energy_source", inplace=True)

        calc_df = pd.merge(
            calc_df,
            pd.DataFrame([share_of_subsector_dict], index=["new_subsector_share"]).T,
            left_index=True,
            right_index=True,
        )
        calc_df["new_share"] = (
            calc_df["new_sector_share"] * calc_df["new_subsector_share"]
        )
