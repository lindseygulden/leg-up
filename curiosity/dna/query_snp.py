...
def get_and_validate_config(config_file_path: Union[str, PosixPath]) -> dict:
    ...
    schema = sch.Schema(
        {
            "locations": sch.And(list, lambda x: all(isinstance(xx, str) for xx in x)),
            "api_key": sch.And(str),
            sch.Optional(
                "entry_point",
                default="https://api.worldweatheronline.com/premium/v1/past-weather.ashx",
            ): sch.And(str, lambda x: validators.url(x)),
            "end_date": sch.And(str, lambda x: len(x) == 11),
            "start_date": sch.And(str, lambda x: len(x) == 11),
            sch.Optional("timeout_seconds", default=30): sch.And(int, lambda t: t > 0),
            sch.Optional("frequency", default=1): sch.And(
                lambda f: f in [1, 3, 6, 12, 24], int
            ),
            "daily_weather_variables": sch.And(
                list,
                lambda x: all(
                    xx
                    in [
...
def extract_data(
    ...
    for _, row in weather.iterrows():
        weather_dict = {k: row[k] for k in tuple(daily_weather_variables)}
        weather_dict.update(
            {k: row["astronomy"][0][k] for k in tuple(astronomy_variables)}
        )
        for hr in range(len(row["hourly"])):
            weather_dict.update(
                {k: row["hourly"][hr][k] for k in tuple(hourly_variables)}
            )
            if extract_weather_desc:
                weather_dict.update(
                    {"weatherDesc": row["hourly"][hr]["weatherDesc"][0]["value"]}
                )
